...
from app.routes.linebots.schema import (
    PushMessageRequest,
    PushMessageResponse,
    MessageQuotaResponse,
)

router = APIRouter(tags=["linebots"])


@router.get("/linebots/profile/{user_id}", response_model=Contact)
...
@router.get("/linebots/message-quota", response_model=MessageQuotaResponse)
